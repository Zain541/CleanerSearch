...
<form class="form-horizontal" id="edit_property_type">
    ...
    <div class="form-group">
      ...
      <div class="col-sm-10">
        <input type="text" class="form-control" value="{{ $propertytype->name }}" id="name" placeholder="Enter Name" name="name">
      </div>
    ...
    <div class="form-group">        
      <div class="col-sm-offset-2 col-sm-10">
        ...
        <a href="{{ route('propertytypes.index') }}" class="btn btn-primary">Cancel</a>
      </div>
    </div>
  </form>
  </div>
  ...
  $(document).ready(function(){
    $("#edit_property_type").submit(function(e){
      ...
      var name = $("#name").val();
       $.ajax({
                    url: '{{ route("propertytypes.update", $propertytype->id)}}',
                    type: 'PUT',
                    ...
                    success: function (data) { 
                      if(data.msg == "true")
                       {
                          window.location.replace("{{ route('propertytypes.index') }}");
                       }
